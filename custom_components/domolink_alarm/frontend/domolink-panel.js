class DomolinkPanel extends HTMLElement {
  ...
  _bindEvents() {
    ...
    const brandSec = this.querySelector('.brand-section');
    if (brandSec) {
      brandSec.style.cursor = 'pointer';
      let brandClicks = [];
      brandSec.addEventListener('click', () => {
        const now = Date.now();
  ...
  _syncSidebarBadge(hasUpdate) {
    try {
      const isAlarmUpdate = Boolean(hasUpdate);
